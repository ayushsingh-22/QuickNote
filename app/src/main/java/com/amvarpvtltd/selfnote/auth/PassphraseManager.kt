...
object PassphraseManager {
    ...
    fun generateQRCode(passphrase: String): Bitmap {
        ...
        val deepLink = "quicknote://sync?passphrase=$passphrase"
    ...
    fun extractPassphraseFromQR(qrContent: String): String? {
        try {
            ...
            // If a quicknote deep link appears anywhere (maybe embedded), extract passphrase
            try {
                val idx = content.indexOf("quicknote://sync?passphrase=", ignoreCase = true)
                if (idx >= 0) {
                    val substr = content.substring(idx)
                    val extracted = substr.substringAfter("passphrase=", "").substringBefore("&").trim()
                    if (extracted.isNotEmpty()) {
                        Log.d(TAG, "Extracted passphrase from embedded quicknote link: $extracted")
                        return extracted
                    }
                }
                // Also handle percent-encoded quicknote (quicknote%3A%2F%2F...)
                val pctIdx = content.indexOf("quicknote%3A", ignoreCase = true)
                if (pctIdx >= 0) {
                    ...
                    try {
                        ...
                        if (extracted.isNotEmpty()) {
                            Log.d(TAG, "Extracted passphrase from percent-encoded quicknote link: $extracted")
                            return extracted
                        }
                    } catch (e: Exception) {
                        Log.d(TAG, "Percent-decode of embedded quicknote failed", e)
                    }
                }
            } catch (e: Exception) {
                Log.d(TAG, "Embedded quicknote extraction failed", e)
            }
